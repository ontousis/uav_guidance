...
def new_speed(old_vx_uav, old_vy_uav, dvx_lst, dvy_lst, positions_lst, old_mpp_x, old_mpp_y, times_lst, veh_length, connection, center_x, center_y, ac_x, ac_y, last_added_cx, last_added_cy):
	# old_vx_uav: previous uav speed in m/s (+ -> forward)
	# old_vy_uav: previous uav speed in m/s (+ -> right)
	# veh_length: avg side length of target vehicle in pixels
	# positions_lst: list of tupples (x,y) with the target's locations - x,y refer to the position in the frame (y,x for the uav)-init with 2 elements
	# dvx_lst: list of speed changes of uav (m/s) - position 1 corresponds to pos 1 of times_lst - initialized as [-1].
	# times_lst: list of timestamps of the positions_lst in s
	avg_side_len=3
	small_threshold=0.5
	exclude_threshold=veh_length/avg_side_len
	centering_thr=100
	centering_seconds=5
	if old_vx_uav==-1:
		mpp=avg_side_len/veh_length #meters per pixel approximation at target location
		new_vx_uav=-mpp*(positions_lst[-1][1]-positions_lst[0][1])/(times_lst[-1]-times_lst[0]) # - because x increases in the oposite direction for the uav
		new_vy_uav=mpp*(positions_lst[-1][0]-positions_lst[0][0])/(times_lst[-1]-times_lst[0])
		mps_per_pps_x=mpp
		mps_per_pps_y=mpp
		dvx_lst.append(new_vx_uav)
		dvy_lst.append(new_vy_uav)
	else:
		cx=0
		cy=0
		excl_change_vx=0
		...
		mps_per_pps_x=0
		mps_per_pps_y=0
		for i in range(max(2,len(positions_lst)-4),len(positions_lst)): # exclude first position (speed assigned to 2nd and later)
			vx_t_p=-(positions_lst[i-1][1]-positions_lst[i-2][1])/(times_lst[i-1]-times_lst[i-2])
			vy_t_p=(positions_lst[i-1][0]-positions_lst[i-2][0])/(times_lst[i-1]-times_lst[i-2])
			vx_t=-(positions_lst[i][1]-positions_lst[i-1][1])/(times_lst[i]-times_lst[i-1])
			vy_t=(positions_lst[i][0]-positions_lst[i-1][0])/(times_lst[i]-times_lst[i-1])
			dvx_t=vx_t-vx_t_p
			dvy_t=vy_t-vy_t_p
			if abs(dvx_t)>exclude_threshold and abs(dvx_lst[i-1])<small_threshold:
				excl_change_vx=1
			else:
				excl_change_vx=0
			if abs(dvy_t)>exclude_threshold and abs(dvy_lst[i-1])<small_threshold:
				excl_change_vy=1
			else:
				excl_change_vy=0
			if dvx_t!=0 and excl_change_vx!=1:
				print("i:",i,"dvx_lst[i-1]:",dvx_lst[i-1],"dvx_t:",dvx_t)
				mps_per_pps_x+=-dvx_lst[i-1]/dvx_t
				cx+=1
			if dvy_t!=0 and excl_change_vy!=1:
				print("i:",i,"dvy_lst[i-1]:",dvx_lst[i-1],"dvy_t:",dvx_t)
				mps_per_pps_y+=-dvy_lst[i-1]/dvy_t
				cy+=1
		if cx!=0:
			mps_per_pps_x=mps_per_pps_x/cx
		else:
			mps_per_pps_x=old_mpp_x
		if cy!=0:
			mps_per_pps_y=mps_per_pps_y/cy
		else:
			mps_per_pps_y=old_mpp_y
		if mps_per_pps_x==0:
			mps_per_pps_x=old_mpp_x
		if mps_per_pps_y==0:
			mps_per_pps_y=old_mpp_y
		mps_per_pps_x=(mps_per_pps_x+avg_side_len/veh_length)/2
		mps_per_pps_y=(mps_per_pps_y+avg_side_len/veh_length)/2
		added_centering_speed_x=0
		added_centering_speed_y=0
		if abs(positions_lst[-1][0]-center_x)>centering_thr:
			if ac_y==0:
				dy=positions_lst[-1][0]-center_x 	# frame x distance is uav y distance
				dym=dy*mps_per_pps_y 			#estimated distance in meters
				added_centering_speed_y=dym/centering_seconds
				last_added_cy=added_centering_speed_y
				ac_y=1
			elif ac_y==1:
				ac_y=2
		else:
			ac_y=-1
		if abs(positions_lst[-1][1]-center_y)>centering_thr:
			if ac_x==0:
				dx=-(positions_lst[-1][1]-center_y) 	# frame y distance is -(uav x distance)
				dxm=dx*mps_per_pps_x			#estimated distance in meters
				added_centering_speed_x=dxm/centering_seconds
				last_added_cx=added_centering_speed_x
				ac_x=1
			elif ac_x==1:
				ac_x=2
		else:
			ac_x=-1
		current_vx=-(positions_lst[-1][1]-positions_lst[-2][1])/(times_lst[-1]-times_lst[-2])
		current_vy=(positions_lst[-1][0]-positions_lst[-2][0])/(times_lst[-1]-times_lst[-2])
		if ac_x==0 or ac_x==1:
			new_vx_uav=old_vx_uav+mps_per_pps_x*current_vx+added_centering_speed_x
			dvx_lst.append(mps_per_pps_x*current_vx+added_centering_speed_x)
		elif ac_x==2:
			new_vx_uav=old_vx_uav+mps_per_pps_x*current_vx+last_added_cx
			dvx_lst.append(mps_per_pps_x*current_vx+last_added_cx)
		elif ac_x==-1:
			new_vx_uav=old_vx_uav+mps_per_pps_x*current_vx
			dvx_lst.append(mps_per_pps_x*current_vx)
			ac_x=0
			last_added_cx=0
		if ac_y==0 or ac_y==1:
			new_vy_uav=old_vy_uav+mps_per_pps_y*current_vy+added_centering_speed_y
			dvy_lst.append(mps_per_pps_y*current_vy+added_centering_speed_y)
		elif ac_y==2:
			new_vy_uav=old_vy_uav+mps_per_pps_y*current_vy+last_added_cy
			dvy_lst.append(mps_per_pps_y*current_vy+last_added_cy)
		elif ac_y==-1:
			new_vy_uav=old_vy_uav+mps_per_pps_y*current_vy
			dvy_lst.append(mps_per_pps_y*current_vy)
			ac_y=0
			last_added_cy=0
	#set_velocity(new_vx_uav,new_vy_uav,connection)
	return (new_vx_uav, new_vy_uav, mps_per_pps_x, mps_per_pps_y, ac_x, ac_y, last_added_cx, last_added_cy)
